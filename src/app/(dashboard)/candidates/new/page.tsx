...
const inputClass = "w-full rounded-lg border border-theme-border3 px-3 py-2 text-sm text-theme-text placeholder-theme-text20 bg-theme-input focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200";
  ...
  return (
    <div className="p-8 max-w-2xl">
      <div className="flex items-center gap-3 mb-8">
        ...
        <h1 className="text-2xl font-bold text-theme-text">Add Candidate</h1>
      </div>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-medium text-theme-text80 mb-1">First Name *</label>
                <input name="firstName" required value={form.firstName} onChange={handleChange} className={inputClass} placeholder="Jane" />
              </div>
              <div>
                <label className="block text-xs font-medium text-theme-text80 mb-1">Last Name *</label>
                <input name="lastName" required value={form.lastName} onChange={handleChange} className={inputClass} placeholder="Doe" />
              </div>
            </div>

            <div>
              <label className="block text-xs font-medium text-theme-text80 mb-1">Email *</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-medium text-theme-text80 mb-1">Phone</label>
                <input name="phone" type="tel" value={form.phone} onChange={handleChange} className={inputClass} placeholder="+1 555 000 0000" />
              </div>
              <div>
                <label className="block text-xs font-medium text-theme-text80 mb-1">Location</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-medium text-theme-text80 mb-1">Current Title</label>
                <input name="currentTitle" value={form.currentTitle} onChange={handleChange} className={inputClass} placeholder="Software Engineer" />
              </div>
              <div>
                <label className="block text-xs font-medium text-theme-text80 mb-1">Current Company</label>
                <input name="currentCompany" value={form.currentCompany} onChange={handleChange} className={inputClass} placeholder="Acme Corp" />
              </div>
            </div>

            <div>
              <label className="block text-xs font-medium text-theme-text80 mb-1">LinkedIn URL</label>
            ...
            <div>
              <label className="block text-xs font-medium text-theme-text80 mb-1">Source</label>
            ...
            <div>
              <label className="block text-xs font-medium text-theme-text80 mb-1">Skills</label>
              ...
              {form.skills.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {form.skills.map(skill => (
                    <span key={skill} className="flex items-center gap-1 rounded-full bg-indigo-500/10 text-indigo-300 px-2.5 py-0.5 text-xs font-medium">
                      {skill}
            ...
            <div>
              <label className="block text-xs font-medium text-theme-text80 mb-1">Summary</label>
